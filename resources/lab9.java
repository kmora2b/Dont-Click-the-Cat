...
public class lab9 {
	...
	public static void main (String args []) { 
		...
		switch(sPromptUser) {
			case "1":	
				...
				while (priceSum == 0); //If priceSum is equal to 0 then repeat 
				...
				sPromptUser = Questions(yesNoPrice);	//Assign sPromptUser to the questions method that takes in the string parameter: yesNoPrice
	...
	public static String canAfford (double budget, double itemPrice) {
		...
		//If the budget is less than or equal to itemPrice then yesNoPrice is assigned to the string "no"
		else if (budget <= itemPrice)
			yesNoPrice = "no";
		
		//Otherwise, assign yesNoPrice to the string ""
		else 
			yesNoPrice = "";
		
		System.out.println("Can you comfortably afford it?\n" + yesNoPrice); //Display if the user can comfortably afford the item and show yesNoPrice
		return yesNoPrice;	//Return yesNoPrice
	}
